...
pub async fn file_to_match_settings(path: String) -> IoResult<flat::MatchSettingsT> {
    ...
    let toml = file_str.parse::<toml::Table>().unwrap_or_else(|_| empty_map.clone());
    ...
    for car in &cars_header[0..cars_len] {
        ...
        let config_toml = config.parse::<toml::Table>().unwrap_or_else(|_| empty_map.clone());

        let settings_header = config_toml["settings"].as_table().unwrap_or(&empty_map);
        let name = settings_header["name"].as_str().unwrap_or_default().to_string();
